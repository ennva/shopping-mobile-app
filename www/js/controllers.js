angular.module('app.controllers', [])
	...
	.controller('menuCtrl', function($scope, $http, sharedCartService, sharedFilterService) {
		...
		var menu_items_mock = [
			...
			},
			{
				...
				"description" : "Product Description",
				"category" : "1",
				"imageId" : "big_spicy_chicken_wrap",
				"price" : 172,
				"available" : true,
				"stock" : 1
			},
	...
	.controller('loginCtrl', function($scope, $http, $ionicPopup, $state, $ionicHistory) {
		...
		$scope.login = function() {
			str = "http://ennvafoodcart.epizy.com/user-details.php?e=" + $scope.user.email + "&p=" + $scope.user.password;
			$http.get(str)
	...
	.controller('signupCtrl', function($scope, $http, $ionicPopup, $state, $ionicHistory) {
		...
		$scope.signup = function(data) {
			...
			var link = 'http://ennvafoodcart.epizy.com/signup.php';
			$http.post(link, {
				n : data.name,
				un : data.username,
				ps : data.password,
