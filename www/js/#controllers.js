...
.controller('loginCtrl',['ApiEndpoint', function($scope,$http,$ionicPopup,$state,$ionicHistory,ApiEndpoint) {
		$scope.user = {};
		
		$scope.login = function(ApiEndpoint) {
			str=ApiEndpoint.url+"/user-details.php?e="+$scope.user.email+"&p="+$scope.user.password;
			$http.get(str)
...
.controller('signupCtrl',['ApiEndpoint',function($scope,$http,$ionicPopup,$state,$ionicHistory,ApiEndpoint) {

	$scope.signup=function(data,ApiEndpoint){
			
			var link = ApiEndpoint.url+'/signup.php';
